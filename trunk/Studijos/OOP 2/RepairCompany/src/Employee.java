...
public class Employee extends Person implements Runnable
{
    ...
    {
        Client owner;
        PrintListStream.println(getInfo() + " tries to repair");
        while ((workedTimeOnCurrentJob <= requiedTimeToEndCurrentJob) && (!fail))
        {
	...
	if (personalNo.compareTo("") == 0)
	{
	    ...
	    PrintListStream.println("Sorry this person already has his personal no");
	}
    }
	...
	if (qualification > 0)
	{
	    ...
	    PrintListStream.println("Sorry employee qualification can't be lower");
	}
    ...
    public String getInfo()
    {
        if (isFree())
        {
           return name + " " + surname + " (personalNo: " + personalNo + "; qualification: " + qualification + "; Free) ";
        } else {
           return name + " " + surname + " (personalNo: " + personalNo + "; qualification: " + qualification + "; Works) "; 
        }   
    }
    ...
    public boolean setJob(Repairable job)
    {
        if (isFree())
        {
            ...
            PrintListStream.println("Our employee " + getInfo() + " gets new technique to repair. It's called " + job.getDescription());
            return true;
        } else {
            PrintListStream.println("Our employee" + getInfo() + " already has job to do");
            return false;
        }
    }
