...
$tquery = "SELECT id, name, picture, description, parent from gallery where id = $id";
$result = mysql_query($tquery, $dbcon);
$row = mysql_fetch_object($result);

if ($id > 1)
{
  print("<a href = 'galleryS.php?GID=$row->parent'><==Grizti</a><BR>\n");
}
...
$tquery = "SELECT id, name, picture, description from gallery where parent = $id";
$result = mysql_query($tquery, $dbcon);
$kiek = 0;
while ($row = mysql_fetch_object($result))
{
  $kiek++;
  print "<th>\n";
  print "<a href = 'galleryS.php?GID=$row->id'>\n";
  print "$row->name<BR>\n";
...
$tquery = "SELECT count(*) as ksize from pictures where parent = $id";
...
$tquery = "SELECT id, filename, small, description from pictures where parent = $id order by id desc";
$result = mysql_query($tquery, $dbcon);

$n = 0;
$p = 0;
print "<table align='center'><tr align='center'><table align='center'>";
while ($row = mysql_fetch_object($result))
{
  $n++;
  if (($n > ($pg - 1) * $divider) && ($n <= $pg * $divider))
  {
    $p++;
    print "<td><center><img alt='nera' title = '$row->description' onclick = 'zoom(this);' src = '$row->small'><BR>$ksize</center></td>\n";
	if ($p == 3)
	{
	  $p = 0;
	  print "<BR></tr></table><table><tr align='center'>";
	}
  }
  $ksize--;
}
...
{
$k = $pg - 1;
print "<a href = 'galleryS.php?GID=$id&page=$k'>Ankstesnis</a>\n";
}
...
if ($n > $divider)
{
  ...
  for ($i=1;$i<=$j;$i++)
  {
    if ($i == $pg)
	{
      ...
      print "<a href = 'galleryS.php?GID=$id&page=$i'>$i|</a>\n";
	}
  }
}
if ($n > $pg * $divider)
{
$k = $pg + 1;
print "<a href = 'galleryS.php?GID=$id&page=$k'>Sekantis</a>\n";
}
