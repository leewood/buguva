...
class TextConsole extends JTextPane {
	 ...
	 */
	private List<FormRange> formRanges = new ArrayList<FormRange>();
    ...
    public void delline(int i)
    {
    	...
    	{    	
    	   ...
    	   int start = (widthChars + 1) * (i - 1);
    	   int count = widthChars + 1;
    	   StringBuilder buffer = new StringBuilder(text);    	   
           buffer.delete(start, start + count - 1);
	...
	public void backColor(int i, int j, Color color) {
		StyleConstants.setBackground(attrs, color);
		getStyledDocument().setCharacterAttributes(i, j - i,
		attrs, true);
	}	

    private Color localBackground = Color.BLACK;
    
    public void setLocalBackground(Color color)
    {
    	localBackground = color;
    }

    public Color getLocalBackground()
    {
	...
	private List<Integer> keysMap = new ArrayList<Integer>(); 
	private boolean useExtendedKeyEvent = false;
	
	public void setNormalKeyboradMode()
	{
		useExtendedKeyEvent = false;
	}
	
	public void setExtendedKeyboardMode()
	{
		useExtendedKeyEvent = true;
	}
	
	private int keyModif = 0; 
	
	protected void processKeyEventMode2(KeyEvent e)
	{	
		int id = e.getID();
		if (id == e.KEY_PRESSED)
		{
    	   ...
    	   System.out.println("Processing:" + keyCode);
    	   if (keyCode != e.VK_SHIFT)
    	   {
    	   
		   synchronized (keysMap)
		   {		
		       keysMap.add(new Integer(keyCode));
		   }
    	   } else
    	   {
    	   	  keyModif = 2000;
	...
	public int getch()
	{
		...
		{						
			int code = ((Integer)keysMap.get(0)).intValue();
			keysMap.remove(0);
			return code + keyModif;
		}
		}
	} 
	...
	{
		if (useExtendedKeyEvent)
		{
			processKeyEventMode2(e);
		} else 
		{		
	...
	public void write(String string) {
		...
		backColor(start, end, getLocalBackground());
	}
